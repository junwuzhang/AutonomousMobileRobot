...
void RobotControl(void) {
	...
	if (settlegyro < SETTLETIME) {
		...
		if(numpels_blue > 17 && resetflag && ifblue) {
		...
		else if(numpels_orange > 17 && resetflag && (!ifblue)) {
			//			distance = 0.00144*cen_row*cen_row + 0.194*cen_row + 8.07;
			//			obj_x = ROBOTps.x + distance*cos(ROBOTps.theta);
			//			obj_y = ROBOTps.y + distance*sin(ROBOTps.theta);
			if((ROBOTps.y<12.0 && ROBOTps.y>0.5) && (ROBOTps.x>-6.0 && ROBOTps.x<6.0)) {
			...
			if(astarrunning==0){
				...
				if( xy_control(&vref, &turn, 1.0, ROBOTps.x, ROBOTps.y, (-5+pathCol[(pathLen - 1) - current_index]),(11 - (pathRow[(pathLen - 1) - current_index])), ROBOTps.theta, 0.25, 0.5))
				{
					...
					if(current_index == pathLen) {
						...
						astarrunning = 1;
						SEM_post(&SEM_startAstar);
						if(ROBOTps.x-robotdest[7].x<0.5 && ROBOTps.y-robotdest[7].y<0.5){
							whichcase = 3;
						}
					}
				}
...
//					else {								// working 80%
//					if ((countdelayblue >= 501) && (gateclose = 0)) {
						vref = 0.50;
						turn = kp*(0-cen_col_blue);
						if(numpels_blue>45){
							Gate = gateOpen;
...
//			}
		...
		case 3:
			...
			vref = 0;
			turn = 0;
			if(countdrop > 750 && countdrop < 1250){
				if(statePos == 7)
					Switch = switchBlue;
				else
					Switch = switchOrange;
			}
			if(countdrop > 1750 && countdrop < 2000)
								Gate = gateOpen;
			if(countdrop >2000 && countdrop < 7000){
				vref = -1.8;
			}
			if(countdrop > 3000){
				countdrop = 0;
