...
int blue_numpixs = 0;
float green_rcenter = 0;
float green_ccenter = 0;
int green_numpixs = 0;
...
//					green_ccenter = cbar - IMAGE_COLUMNS/2;
//					green_rcenter = rbar - IMAGE_ROWS/2;
//					green_numpixs = final_object_stats[largest_object].num_pixels_in_object;
...
//			}
		...
		if(ifblue) {	// if ifblue = 1, scan blue color
			...
			specs_v = 171;
			specs_vrad = 65;
		} else {	// if ifblue = 0, scan green color
			specs_h = 120;
			specs_hrad = 35;
			if((specs_h-specs_hrad)<0) // wrap 0->360
			{
