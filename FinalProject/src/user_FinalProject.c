...
int numpels_blue = 0;
int numpels_orange = 0;
float cen_row_blue = 0;
float cen_col_blue = 0;
float cen_row_orange = 0;
float cen_col_orange = 0;
...
int count2 = 0;
int countdelay = 0;
int countdelayblue = 0;
int countdelayorange = 0;
int countswitch = 0;
int delayblueswitch = 0;
int delayorangeswitch = 0;
int gateclose = 0;
int whichcase = 0;

float gateClose = 12.5;
float gateOpen = 10.0;
float switchBlue = 5.5; // Ready to collect or drop blue balls
float switchOrange = 9.2; // Ready to collect or drop orange balls
float Gate = 12.5; // PWM3
float Switch = 6.3; // PWM4
...
int whichball = 0; // 1: orange ball  2: blue ball
	...
	while(1) {
		...
		if ((tskcount%6)==0) {
			if (GET_LVDATA_TO_LINUX) {
				...
				if (sendball) {
					var5 = ballposx;	// Send x position of ball
					var6 = ballposy;	// Send y position of ball
					var7 = whichball;	// Send which color of ball
...
//
				...
				{
					var7 = 0;	// Ensures no ghost balls present
				}
				// Default
				//ptrshrdmem->DSPSend_size = sprintf(toLinuxstring,"1.0 1.0 1.0 1.0");
				// you would do something like this
				ptrshrdmem->DSPSend_size = sprintf(toLinuxstring,"%.1f %.1f %.1f %.1f %.1f %.1f %.1f %.1f",var1,var2,var3,var4,var5,var6,var7,var8);
...
Void main()
{
   ...
   // on power up wait until Linux has initialized Timer1
	...
	//Stop
	robotdest[8].x = -1;		robotdest[8].y = -1;
...
void RobotControl(void) {
	...
	if (settlegyro < SETTLETIME) {
		...
		// Get the pixel and coordinate datas of the balls
		if(new_coordata) {
			if(ifblue) {
				cen_row_blue = blue_rcenter;
				cen_col_blue = blue_ccenter;
				numpels_blue = blue_numpixs;
			} else {
				cen_row_orange = orange_rcenter;
				cen_col_orange = orange_ccenter;
				numpels_orange = orange_numpixs;
			}
			new_coordata = 0;
		}

		// Check if a ball can be pursued
		// This is for blue (pixels > 12)
		if(numpels_blue > 10 && resetflag && ifblue) {
			if((ROBOTps.y<12.0 && ROBOTps.y>0.5) && (ROBOTps.x>-6.0 && ROBOTps.x<6.0)) {	// Check if in course
					seeball = 1;	// We see a ball
					whichcase = 1;	// Blue ball case
					countblue = 0;	// Zero the blue delay
			}
		}

		// This is for orange (pixels > 12)
		else if(numpels_orange > 10 && resetflag && (!ifblue)) {
			if((ROBOTps.y<12.0 && ROBOTps.y>0.5) && (ROBOTps.x>-6.0 && ROBOTps.x<6.0)) {	// Check if in course
					seeball = 1;	// We see a ball
					whichcase = 2;	// Orange ball case
					countorange = 0;	// Zero the orange ball
			}
		}
		else {
			seeball = 0;	// Else we don't see a ball
		}

		if(counterstorm>0) {	// For the case that ladar detects obstacle when pursuing a ball
			counterstorm--;	// A counter to delay pursuing ball again
			if(counterstorm==0)
				resetflag = 1;
		}

		if(counterladar>0) {	// Delay for ladar running too fast
			counterladar--;
		}

		switch(whichcase) {
		// Case 0 is case of A-Star
		case 0:
			Gate = gateClose;
			if(astarrunning==0){
				...
				if( xy_control(&vref, &turn, 1.0, ROBOTps.x, ROBOTps.y, (-5+pathCol[(pathLen - 1) - current_index]),(11 - (pathRow[(pathLen - 1) - current_index])), ROBOTps.theta, 0.25, 0.5))
				{
					...
					if(current_index == pathLen) {
						statePos = (statePos+1)%robotdestSize;	// Go to next index when reached target
						startrow = 11-roundf(ROBOTps.y);	// Send all initial and end states
						startcol = roundf(ROBOTps.x)+5;
						endrow = 11-robotdest[statePos].y;
						endcol = robotdest[statePos].x+5;
						astarrunning = 1;
						SEM_post(&SEM_startAstar);
						if(ROBOTps.x-robotdest[7].x<0.5 && ROBOTps.y-robotdest[7].y<0.5){	// Case for dropping off balls
							whichcase = 3;
						}
					}
				}
				...
				vref = 0;	// If A-Star is running, stall car
				turn = 0;
			}
			...
			if (newLADARdata == 1) {
				...
				// Detect obstacles by utilizing a mirrored map that will use ladar distances to plot if
				// an object is detected for 10 times at the same location
				for(i=20; i<205; i+=3) {
					if((LADARdataY[i]<12.0 && LADARdataY[i]>-0.5) && (LADARdataX[i]>-6.0 && LADARdataX[i]<6.0) && LADARdistance[i]<500) {
						...
						if(obstaclemap[11*rowmap + colmap] > 10) {
							if(map[11*rowmap + colmap] == '0') {
								...
								endcol = robotdest[statePos].x+5;
								astarrunning = 1;
								SEM_post(&SEM_startAstar);	// Re-run A-Star when new obstacle on map
							}
						}
					}
				}
		...
		case 1:					// Gather Blue Ball
			if (newLADARdata == 1 && (counterladar==0)) {
				newLADARdata = 0;
				for (i=0;i<228;i++) {						// Pass new LADAR data
					LADARdistance[i] = newLADARdistance[i];
					LADARangle[i] = newLADARangle[i];
					LADARdataX[i] = newLADARdataX[i];
					LADARdataY[i] = newLADARdataY[i];
				}
				// Check if ladar data too close
				// Don't go towards ball if obstacle in way
				for (i=30;i<200;i+=3) {
					if(LADARdistance[i]<445) {
						seeball = 0;
						vref = 0;
						turn = 0;
						whichcase = 0;
...
//						obsdetected = 1;
						...
						resetflag = 0;
						counterladar = 3500;
						minimum = LADARdistance[i];
						SEM_post(&SEM_startAstar);	// Re-run A-Star if obstacle in way
					}
				}
			}

				// Delay the car for half a second before going towards the ball
				if(seeball) {
					delayblueswitch++;
					countdelayblue++;
					if(delayblueswitch==300)
						Switch = switchBlue;
					if(countdelayblue < 500) {
						vref = 0;
						turn = kp*(0-cen_col_blue);
					}
					else {								// working 80%
						vref = 0.90;
						turn = kp*(0-cen_col_blue);
				...
				// Close the gate when the ball is close enough
				// Run towards ball for 1.3 seconds at vref = 1.4
				if(gateclose) {
					turn = kp*(0-cen_col_blue);
					vref = 1.4;
...
//						send_to_labview = 1;
		...
		// Case is very similar to blue ball except utilizing orange ball variables
		case 2: // Get Orange Ball
			if (newLADARdata == 1 && (counterladar==0)) {
				newLADARdata = 0;
				for (i=0;i<228;i++) {
					LADARdistance[i] = newLADARdistance[i];
...
//			if(obsdetected==0){
				if(seeball) {
					delayorangeswitch++;
					countdelayorange++;
					if(delayorangeswitch==300){
						Switch = switchOrange;
					}

					if(countdelayorange < 500) {
						vref = 0;
						turn = kp*(0-cen_col_orange);
					}
//					else if(!gateclose) {
					else {
						vref = 0.90;
						turn = kp*(0-cen_col_orange);
						if(numpels_orange>45){
							Gate = gateOpen;
							gateclose = 1;
						}
					}
...
//			}
		...
		// Case  to drop off balls
		case 3:
			countdrop++;
			vref = 0;
			turn = 0;
			if(countdrop > 750 && countdrop < 1250){	// Stall car for .75 seconds
				if(statePos == 7)
					Switch = switchBlue;
				else
					Switch = switchOrange;
			}
			if(countdrop > 1750 && countdrop < 2000)	// Then open gate slowly
								Gate = gateOpen;
			if(countdrop >2000 && countdrop < 7000){	// Finally drop
				vref = -1.8;
			}
			if(countdrop > 3000){
				countdrop = 0;
				Gate = gateClose;
				if(statePos == 8)	// Last case
					whichcase = 4;
				else whichcase = 0;
			}
			break;

		// Last case where robot will stop
		// Its course has finished
		case 4:
			vref = 0;
			turn = 0;
			break;
		...
		if ((timecount%200)==0) {
//			LCDPrintfLine(1,"%.1f %.f %.1f %i %i",ROBOTps.x,ROBOTps.y, Switch, whichcase, seeball);
			LCDPrintfLine(1,"%.1f ",Switch);
			LCDPrintfLine(2,"t:%.1f,f:%.1f %.1f %.1f",ROBOTps.theta,previous_frame, minimum, turn);
		}


		SetRobotOutputs(vref,turn,0,Gate,Switch,0,0,0,0,0);
		//SetRobotOutputs(vref,turn,0,0,0,Switch,Gate,0,0,0);
