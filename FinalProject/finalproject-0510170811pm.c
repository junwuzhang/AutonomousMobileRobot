...
//					var4 = ROBOTps.theta;
				...
				ptrshrdmem->DSPSend_size = sprintf(toLinuxstring,"%.1f %.1f %.1f %.1f %.1f %.1f",var1,var2,var3,var4,var5,var6);
...
void RobotControl(void) {
	...
	if (settlegyro < SETTLETIME) {
		...
		if(numpels_blue > 12 && resetflag && ifblue) {
		...
		else if(numpels_orange > 12 && resetflag && (!ifblue)) {
			//			distance = 0.00144*cen_row*cen_row + 0.194*cen_row + 8.07;
			//			obj_x = ROBOTps.x + distance*cos(ROBOTps.theta);
			//			obj_y = ROBOTps.y + distance*sin(ROBOTps.theta);
			if((ROBOTps.y<12.0 && ROBOTps.y>0.5) && (ROBOTps.x>-6.0 && ROBOTps.x<6.0)) {
...
//						if(ROBOTps.x-robotdest[6].x<0.5 && ROBOTps.y-robotdest[6].y<0.5){
							whichcase = 3;
						}
					}
				}
