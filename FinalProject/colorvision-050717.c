...
int ifblue = 1;
float blue_rcenter = 0;
float blue_ccenter = 0;
int blue_numpixs = 0;
float orange_rcenter = 0;
float orange_ccenter = 0;
int orange_numpixs = 0;
...
void init_ColorVision(void) {
	...
	// set specs for bright red -- original
//	specs_h = 254;
//	specs_hrad = 8;
		...
		if (ifblue) {
			...
			if (new_coordata == 0) {
				if (final_object_stats[largest_object].num_pixels_in_object > 1) {
					...
					blue_ccenter = cbar - IMAGE_COLUMNS/2;
					blue_rcenter = rbar - IMAGE_ROWS/2;
					blue_numpixs = final_object_stats[largest_object].num_pixels_in_object;
			...
			if (new_coordata == 0) {
				if (final_object_stats[largest_object].num_pixels_in_object > 1) {
					noimagefound = 0;
					new_num_found_objects = num_unique_objects;
					orange_ccenter = cbar - IMAGE_COLUMNS/2;
					orange_rcenter = rbar - IMAGE_ROWS/2;
					orange_numpixs = final_object_stats[largest_object].num_pixels_in_object;
					//new_object_theta 	= final_object_stats[largest_object].theta;
					//new_C20				= final_object_stats[largest_object].C20_sum;
					//new_C02				= final_object_stats[largest_object].C02_sum;
					new_coordata = 1;
...
//		}
		 ...
		 * V_rad = 48
		 */
		if(ifblue) {	// if ifblue = 1, scan blue color
			specs_h = 174;
			specs_hrad = 13;
			if((specs_h-specs_hrad)<0) // wrap 0->360
			{
			...
			specs_v = 127;
			specs_vrad = 50;
		} else {	// if ifblue = 0, scan orange color
			specs_h = 255;
			specs_hrad = 15;
			if((specs_h-specs_hrad)<0) // wrap 0->360
			{
