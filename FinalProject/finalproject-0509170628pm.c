...
void RobotControl(void) {
	...
	if (settlegyro < SETTLETIME) {
		...
		case 3:
			...
			vref = 0;
			turn = 0;
			if(countdrop > 500 && countdrop < 1500){
				if(statePos == 7)
					Switch = switchBlue;
				else
					Switch = switchOrange;
			...
			if(countdrop >1500 && countdrop < 7000){
				vref = -1.2;
			}
			if(countdrop > 2500){
				countdrop = 0;
		...
		if ((timecount%200)==0) {
			LCDPrintfLine(1,"%.1f %.f %.1f %i ",ROBOTps.x,ROBOTps.y, Switch, gateclose);
